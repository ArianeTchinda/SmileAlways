...
import {
  Menu, X, Phone, Calendar, Mail, MapPin, ChevronDown,
  Smile, Crown, Scissors, Zap, Shield, Heart,
} from "lucide-react";
import logoImg from "@/assets/logo.png";

const servicesDropdown = [
  { name: "Soins Préventifs", href: "/services/preventifs", icon: Smile, desc: "Détartrage, examens et prévention" },
  { name: "Prothèses Dentaires", href: "/services/protheses", icon: Crown, desc: "Couronnes, bridges et implants" },
  { name: "Chirurgie Dentaire", href: "/services/chirurgie", icon: Scissors, desc: "Extractions et interventions" },
  { name: "Esthétique Dentaire", href: "/services/esthetique", icon: Zap, desc: "Blanchiment et facettes" },
  { name: "Orthodontie", href: "/services/orthodontie", icon: Shield, desc: "Appareils et aligneurs" },
  { name: "Soins d'Urgence", href: "/services/urgences-dentaires", icon: Heart, desc: "Urgences 24h/24, 7j/7" },
];
...
const Header = () => {
  ...
  const menuItems = [
    ...
    { name: "Équipe", href: "/equipe", hasDropdown: false },
    { name: "Urgences 24/7", href: "/urgences", hasDropdown: false },
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-20">
            ...
            <Link to="/" className="flex items-center space-x-3 group">
              <div className="w-12 h-12 rounded-xl overflow-hidden shadow-md group-hover:shadow-lg transition-shadow duration-300">
                <img src={logoImg} alt="Smile Always Logo" className="w-full h-full object-cover" />
              </div>
                ...
                {menuItems.map((item) =>
                  item.hasDropdown ? (
                    ...
                    >
                      <Link
                        to={item.href}
                        className={`nav-link-underline px-4 py-3 text-sm font-semibold transition-colors duration-200 inline-flex items-center gap-1 ${
                          isActive(item.href)
                    ...
                    <Link
                      key={item.name}
                      to={item.href}
                      className={`nav-link-underline px-4 py-3 text-sm font-semibold transition-colors duration-200 ${
                        isActive(item.href)
                          ? "text-dental-blue active"
                          : "text-foreground hover:text-dental-blue"
                      }`}
          ...
          <div
            className={`lg:hidden overflow-hidden transition-all duration-300 ${
              isMenuOpen ? "max-h-[600px] border-t border-border" : "max-h-0"
            }`}
          >
            <nav className="py-4">
              <div className="flex flex-col">
